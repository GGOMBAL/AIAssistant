        ...
        if res.status_code == 200 and res.json()["rt_cd"] == '0':
            ...
            for stock in ResultList:
                ...
                if int(stock['hldg_qty']) > 0:
                    ...
                    StockInfo["StockCode"] = stock['pdno']              # 코드
                    StockInfo["StockName"] = stock['prdt_name']         # 네임
                    StockInfo["StockAmt"] = stock['hldg_qty']           # 주식 수
                    StockInfo["StockAvgPrice"] = stock['pchs_avg_pric'] # 평균 매수가
                    StockInfo["StockOriMoney"] = stock['pchs_amt']      # 매수 금액
                    StockInfo["StockNowMoney"] = stock['evlu_amt']      # 현재 금액
                    StockInfo["StockNowPrice"] = stock['prpr']          # 현재 가격
                    StockInfo["StockNowRate"] = stock['fltt_rt']        # 등락률
                    StockInfo["Gain"] = stock['evlu_pfls_rt']   # 수익률
                    StockInfo["StockRevenueMoney"] = stock['evlu_pfls_amt'] # 수익금
...
############################################################################################################################################################
#국내 주식 등락률 
def GetCurrentChange(stock_code):
    ...
    if res.status_code == 200 and res.json()["rt_cd"] == '0':
        return float(res.json()['output']['prdy_ctrt'])
    else:
        print("Error Code : " + str(res.status_code) + " | " + res.text)
        return res.json()["msg_cd"]
    
#국내 주식 거래량 변화
def GetCurrentVolChg(stock_code):
    ...
    if res.status_code == 200 and res.json()["rt_cd"] == '0':
        return float(res.json()['output']['prdy_vrss_vol_rate'])
    else:
        print("Error Code : " + str(res.status_code) + " | " + res.text)
        return res.json()["msg_cd"]
...
def MakeSellMarketOrder(stockcode, amt):
    ...
    for stocks in MyStockList:
        if stocks['StockCode'] == stockcode:
            AccntStockCode = stocks['StockCode']
            AccntStockAmt = stocks['StockAmt']
    ...
    if int(Common.GetPrdtNo(Common.GetNowDist())) == 29:
        ...
        res = requests.post(URL, headers=headers, data=json.dumps(data))
        #print("Order [StockCode : " + str(stockcode) + " / StockAmt : " + str(amt) + " ]")
        ##print("Accnt [StockCode : " + str(AccntStockCode) + " / StockAmt : " + str(AccntStockAmt) + " ]")
...
def MakeBuyLimitOrder(stockcode, amt, price, adjustAmt = False, ErrLog = "NO"):
    ...
    if int(Common.GetPrdtNo(Common.GetNowDist())) == 29:
        ...
        print("Order [StockCode : " + str(stockcode) + " / StockAmt : " + str(amt) + " TargetPrice : " + str(price) + " ]")
...
def MakeSellLimitOrder(stockcode, amt, price, ErrLog="YES"):
    ...
    for stocks in MyStockList:
        if stocks['StockCode'] == stockcode:
            AccntStockCode = stocks['StockCode']
            AccntStockAmt = stocks['StockAmt']
            AccntStockPrice = stocks['StockAvgPrice']
    ...
    if int(Common.GetPrdtNo(Common.GetNowDist())) == 29:
        ...
        #print("Order [StockCode : " + str(stockcode) + " / StockAmt : " + str(amt) + " TargetPrice : " + str(price) + " ]")
        ##print("Accnt [StockCode : " + str(AccntStockCode) + " / StockAmt : " + str(AccntStockAmt) + " TargetPrice : " + str(AccntStockPrice) + " ]")
...
def GetOhlcv(stock_code,p_code,start_date,end_date,adVar,ohlcv="Y"):

    if(adVar):
        start_date = Common.GetFromNowDateStr("KR","NONE",-36500),
        end_date = Common.GetNowDateStr("KR")
    else:
    ...
    params = {
        ...
        "FID_ORG_ADJ_PRC": OHLCV_Input #0 : ohlcv
    }
    ...
    if res.status_code == 200 and res.json()["rt_cd"] == '0':
        ...
        if len(pd.DataFrame(ResultList)) > 0:
            ...
            if len(OhlcvList) > 0:
                ...
                listtemp = [ 'open', 'high', 'low', 'close', 'volume', 'divfac']
...
def GetOhlcvMin(stock_code):
    ...
    for i in range(21):
        ...
        if i == 0:
            get_hour = initial_hour
        else:
            get_hour = get_hour + 3000 # end point : 090000 = 152000 - 3000 * 21

        para = {
        "FID_ETC_CLS_CODE": "",
        "FID_COND_MRKT_DIV_CODE": "J",
        "FID_INPUT_ISCD": stock_code,
        "FID_INPUT_HOUR_1": get_hour,
                ...
                for ohlcv in ResultList:
                    ...
                    try:
                        if ohlcv['stck_oprc'] != "":
                            ...
                            OhlcvData['date'] = ohlcv['stck_bsop_date']
                            OhlcvData['time'] = ohlcv['stck_cntg_hour']
                            OhlcvData['open'] = float(ohlcv['stck_oprc'])
                            OhlcvData['high'] = float(ohlcv['stck_hgpr'])
                            OhlcvData['low'] = float(ohlcv['stck_lwpr'])
...
#p_code -> D:일, W:주, M:월, Y:년
def GetSectorOhlcv(stock_code,p_code,start_date,end_date,adVar):
    ...
    params = {
        "FID_PERIOD_DIV_CODE" : "D", #일/주/월 구분코드 ( D:일별 , W:주별, M:월별 )
        "FID_COND_MRKT_DIV_CODE": "U",
        "FID_INPUT_ISCD": stock_code,
        "FID_COND_SCR_DIV_CODE": '20214', #Unique key( 20214 )
        "FID_MRKT_CLS_CODE": "K",   #시장구분코드(K:거래소, Q:코스닥, K2:코스피200)
        "FID_BLNG_CLS_CODE": "0"    #시장구분코드(K:거래소) 0:전업종, 1:기타구분, 2:자본금구분 3:상업별구분 / 시장구분코드(Q:코스닥) 0:전업종, 1:기타구분, 2:벤처구분 3:일반구분 / 시장구분코드(K2:코스닥) 0:전업종
    }
        ...
        if len(pd.DataFrame(ResultList)) > 0:
            ...
            for ohlcv in ResultList:
                ...
                try:
                    ...
                    OhlcvData['close'] = ohlcv['bstp_nmix_prpr'] #업종 지수 현재가
                    OhlcvData['change'] = float(ohlcv['bstp_nmix_prdy_vrss'])                       
                    OhlcvData['AD_Ratio'] = float(ohlcv['ascn_issu_cnt']) / float(ohlcv['down_issu_cnt']) 
                    OhlcvData['volume'] = float(ohlcv['acml_vol'])
                    OhlcvData['ATH'] = float(ohlcv['dryy_bstp_nmix_hgpr'])
                    OhlcvData['ATL'] = float(ohlcv['dryy_bstp_nmix_lwpr'])
            ...
            if len(OhlcvList) > 0:
                ...
                listtemp = [ 'open', 'high', 'low', 'close', 'volume','candle', 'change']
                df[listtemp] = df[listtemp].apply(pd.to_numeric)
...
def GetIndexOhlcv(stock_code):
    ...
    params = {
        "FID_COND_MRKT_DIV_CODE": "U", #일/주/월 구분코드 ( D:일별 , W:주별, M:월별 )
        "FID_INPUT_ISCD": stock_code, #코스피(0001), 코스닥(1001), 코스피200(2001)
    }
    ...
    if res.status_code == 200 and res.json()["rt_cd"] == '0':
        ...
        try:
            OhlcvData['Date'] = strYMD
            OhlcvData['close'] = ResultList['bstp_nmix_prpr'] #업종 지수 현재가
            OhlcvData['change'] = float(ResultList['bstp_nmix_prdy_vrss'])                       
            OhlcvData['AD_Ratio'] = round(float(ResultList['ascn_issu_cnt']) / float(ResultList['down_issu_cnt']) ,2)
            OhlcvData['volume'] = float(ResultList['acml_vol'])
...
def GetAccntTradeHist(mode, start_date, end_date):
    ...
    params = {
        "CANO": Common.GetAccountNo(Common.GetNowDist()),
        "ACNT_PRDT_CD":"01",
        "PDNO": "",#Common.GetPrdtNo(Common.GetNowDist()),
        "INQR_STRT_DT": Common.GetFromNowDateStr("KR","NONE", difference_in_days),
        "INQR_END_DT": Common.GetNowDateStr("KR"),
        "SORT_DVSN":"02",
        "CBLC_DVSN":"00",
        "CTX_AREA_NK100":"",
    ...
    if res.status_code == 200 and res.json()["rt_cd"] == '0':
        ...
        for Result in ResultList:
            ...
            try:
                ...
                if int(Result['pchs_unpr']) > 0 and int(Result['buy_qty']) > 0: 
                    pass
                    #AccntTrdDict = dict()
                    #
                    #AccntTrdDict['Type'] = 'BUY'
                    #AccntTrdDict['Date'] = Result['trad_dt']
                    #AccntTrdDict['StockCode'] = Result['pdno']
                    #AccntTrdDict['BuyPrice'] = Result['pchs_unpr']
                    #AccntTrdDict['SellPrice'] = ""
                    #AccntTrdDict['Amt'] = Result['buy_qty']
                    #AccntTrdDict['slipeage'] = Result['fee']           
                    #AccntTrdDict['StockName'] = Result['prdt_name']
                else:
                    AccntTrdDict = dict()
                    AccntTrdDict['Type'] = 'SELL'
                    AccntTrdDict['Date'] = Result['trad_dt']
                    AccntTrdDict['StockCode'] = Result['pdno']
                    AccntTrdDict['BuyPrice'] = Result['pchs_unpr']
                    AccntTrdDict['SellPrice'] = Result['sll_pric']
                    AccntTrdDict['Amt'] = Result['sll_qty']
                    AccntTrdDict['slipeage'] = Result['fee']           
                    AccntTrdDict['StockName'] = Result['prdt_name']  
                    
                    AccntTrdList.append(AccntTrdDict)
                                      
            except Exception as e:
                print("E:", e)           
        
        result = pd.DataFrame(AccntTrdList)
        print(result)
