...
아무튼 공통 모듈의 경우 변경사항이 생기면 제 블로그에도 내용이 공유가 되니 종종 놀러 오세요!
https://blog.naver.com/zacra

제가 현재 사용하는 여러 최신 전략 등이 있습니다.
'''
...
from Helper.KIS.KIS_Make_StockList import CallStockList 
...
def check_and_refresh_token_if_expired(response):
    """
    토큰 만료 체크 및 자동 재발급
    """
    try:
        if response.status_code != 200:
            ...
            if response_data.get("msg_cd") == "EGW00123":
                print("[TOKEN] Token expired. Refreshing token...")
                logging.warning("Token expired, attempting to refresh")
                
                # 토큰 강제 재발급 (런타임 import로 순환 import 방지)
                print("[TOKEN] Forcing token refresh...")
                
                # Common.MakeToken을 직접 호출하여 강제 토큰 재발급
                try:
                    ...
                    print(f"[TOKEN] Token refresh failed: {token_error}")
                    logging.error(f"Token refresh failed: {token_error}")
                    return False
    except Exception as e:
        print(f"[TOKEN] Error checking token: {e}")
        logging.error(f"Error checking token: {e}")
    
    return False

def make_request_with_token_retry(func, *args, **kwargs):
    """
    토큰 만료시 재시도하는 요청 함수
    """
    max_retries = 2
        ...
        try:
            ...
            if check_and_refresh_token_if_expired(response):
                if attempt < max_retries - 1:
                    print(f"[TOKEN] Retrying request (attempt {attempt + 2}/{max_retries})")
                    continue
            
            return response
            
        except Exception as e:
            print(f"[TOKEN] Request error on attempt {attempt + 1}: {e}")
            if attempt == max_retries - 1:
                raise
...
def MarketOpenType(area):
    
    from datetime import datetime, timezone, time
    
    """
    area == "US" : 미국 주식시장 (프리장/정규장/애프터마켓/Closed)
    area == "HK" : 홍콩 주식시장 (정규장/Closed)
    """
    ...
    if area.upper() == "US":
        ...
        if pre_open <= now_local < reg_open:
            #print("미국 주식 프리장이 열려 있습니다.")
            return "Pre-Market"
        elif reg_open <= now_local < reg_close:
            #print("미국 주식 정규장이 열려 있습니다.")
            return "NormalOpen"
        elif reg_close <= now_local < after_close:
            #print("미국 주식 애프터마켓이 열려 있습니다.")
            return "After-Market"
        else:
            #print("미국 주식시장이 닫혀 있습니다.")
            return "Closed"
        ...
        # 주말(토요일=5, 일요일=6) 체크
        if now_hk.weekday() >= 5:
            print("홍콩 주식시장은 주말에 열리지 않습니다.")
            return "Closed"
        
        # 정규장: 09:30–12:00, 점심 휴장 12:00–13:00, 13:00–16:00
        ...
        if (morning_open <= t < morning_close) or (afternoon_open <= t < afternoon_close):
            print("홍콩 주식 정규장이 열려 있습니다.")
            return "NormalOpen"
        else:
            print("홍콩 주식시장이 닫혀 있습니다.")
            return "Closed"
...
def IsMarketOpen():
    ...
    if IsOpen == True:
        ...
        #파일 경로입니다.
        file_path = MainPath + "json/US_Market_OpenCheck.json"
        try:
...
def GetExrt():
    ...
    TrId = "CTRP6504R"
    if Common.GetNowDist() == "VIRTUAL1" or Common.GetNowDist() == "VIRTUAL2":
         TrId = "VTRP6504R"
...
def GetBalance(st = "USD"):
    ...
    res = make_request_with_token_retry(_make_request)
    #pprint.pprint(res.json())
...
def GetMyStockList(st = "USD"):
    ...
    for i in range(1,5):
        ...
        while DataLoad:
            ...
            # Safely check for tr_cont header
            tr_cont_value = res.headers.get('tr_cont', '')
            if tr_cont_value == "M" or tr_cont_value == "F":
                tr_cont = "N"
            else:
                tr_cont = ""
            ...
            if res.status_code == 200 and res.json()["rt_cd"] == '0':
                ...
                for stock in ResultList:
                    ...
                    if int(stock['ovrs_cblc_qty']) > 0:
                        ...
                        StockInfo["Gain"] = stock['evlu_pfls_rt']
...
def MakeBuyLimitOrderOri(stockcode, amt, price, market, adjustAmt = False):
    ...
    if market == 'NAS':        market = 'NASD'    
    elif market == 'NYS':      market = 'NYSE'    
    elif market == 'AMX':      market = 'AMEX' 

    time.sleep(0.2)

    TrId = "TTTT1002U" #미국 매수
    if Common.GetNowDist() == "VIRTUAL1" or Common.GetNowDist() == "VIRTUAL2":
         TrId = "VTTT1002U" # 미국 매수 (가상)
...
def GetMarketCodeUS(stock_code):
    ...
    return FindMarketFromCode(stock_code)
...
#p_code -> D:일, W:주, M:월 
def GetOhlcv(stock_code, p_code, enddate, ohlcv = "Y"):

    if type(enddate) == str:pass
    else:
...
#    enddate = Common.GetNowDateStr("US")
    
    params = {
        "AUTH": "",
        "EXCD": GetMarketCodeUS(stock_code),
        "SYMB": stock_code,
        "GUBN" : str(gubun),
        "BYMD": enddate,
        "MODP": str(OHLCV_INPUT) # 1 : 수정주가 / 0 : 원주가
    }
            ...
            for ohlcv in ResultList:
                ...
                try:
                    if ohlcv['open'] != "":
                        ...
                        #OhlcvData['divfac'] = float(ohlcv['prtt_rate'])
                        #OhlcvData['divcode'] = float(ohlcv['flng_cls_code'])
...
#당일 분봉 구하기
def GetOhlcvMin(stock_code, Area):
            ...
            for ohlcv in ResultList:
                ...
                try:
                    if ohlcv['last'] != "":
                        ...
                        OhlcvData['date'] = ohlcv['xymd']
                        OhlcvData['time'] = ohlcv['xhms']
                        OhlcvData['open'] = float(ohlcv['open'])
                        OhlcvData['high'] = float(ohlcv['high'])
                        OhlcvData['low'] = float(ohlcv['low'])
                        OhlcvData['close'] = float(ohlcv['last'])
                        OhlcvData['volume'] = float(ohlcv['evol'])
...
def FindMarketFromCode(stocks):
    ...
    NYS_List_file_path, NYS_Delist_file_path = CalFilePath("NYS", 'US', 'Stock')
    
    #파일에 리스트를 저장합니다
    with open(NYS_List_file_path, 'r') as json_file:
        NyseStockList = json.load(json_file)
    
    NasdaqStockList = list()

    NAS_List_file_path, NAS_Delist_file_path = CalFilePath("NAS", 'US', 'Stock')

    #파일에 리스트를 저장합니다
    with open(NAS_List_file_path, 'r') as json_file:
        NasdaqStockList = json.load(json_file)

    AmexStockList = list()
    
    #파일 경로입니다.
    AMX_List_file_path, AMX_Delist_file_path = CalFilePath("AMX", 'US', 'ETF')

    #파일에 리스트를 저장합니다
    with open(AMX_List_file_path, 'r') as json_file:
        AmexStockList = json.load(json_file)

    HkStockList = list()
    
    # #파일 경로입니다.
    # HK_List_file_path, HK_Delist_file_path = CalFilePath("NA", 'HK', 'Stock')

    # #파일에 리스트를 저장합니다
    # with open(HK_List_file_path, 'r') as json_file:
    #     HkStockList = json.load(json_file)
...
def GetTotalBalance():
    ...
    if res.status_code == 200 and res.json()["rt_cd"] == '0':
        ...
        for balanceInfo in result:
            
            if balanceInfo['natn_name'] == '미국' or balanceInfo['natn_name'] == '홍콩' or balanceInfo['natn_name'] == '일본':
                
                natn_money = balanceInfo['itgr_ord_psbl_amt']
                currency = balanceInfo['bass_exrt']
                KRW_money = float(natn_money) * float(currency)
                
                KRW_money_total = KRW_money_total + KRW_money
                                           
    return KRW_money_total

def GetBalanceForAll():
    ...
    if res.status_code == 200 and res.json()["rt_cd"] == '0':
        ...
        # result2 = res.json()['output2']   
        # for data in result2:
        #     if data['crcy_cd'] == "USD":
        #         USD_Currency = float(data['frst_bltn_exrt'])
            
        #     if data['crcy_cd'] == "HKD":
        #         HKD_Currency = float(data['frst_bltn_exrt'])       

        # return Rate
        MyStockList = GetMyStockList()
        
        StockOriMoney = 0
        StockNowMoney = 0
        StockRevMoney = 0
        
        USD_Currency_Temp = YF.get_fx_rate("USDKRW=X")
        HKD_Currency_Temp = YF.get_fx_rate("HKDKRW=X")
        
        USD_Currency = round(USD_Currency_Temp['Close'].iloc[0],2)
        HKD_Currency = round(HKD_Currency_Temp['Close'].iloc[0],2)
        
        print(f"USD/KRW : {USD_Currency} HKD/KRW : {HKD_Currency}")        
              
        if USD_Currency == 0:
            USD_Currency = USD_Currency_Temp
        if HKD_Currency == 0:
            HKD_Currency = HKD_Currency_Temp
            
        ## Seperate NAS/NYS ##
        for stock in MyStockList:
            
            if GetMarketCodeUS(stock['StockCode']) == 'HKS':
                
                #if YF.get_asset_info(ChgTickerName(stock['StockCode'], 'HK')) == 'Stock':
                StockOriMoney = StockOriMoney + float(stock['StockOriMoney']) * HKD_Currency
                StockNowMoney = StockNowMoney + float(stock['StockNowMoney']) * HKD_Currency
                StockRevMoney = StockRevMoney + float(stock['StockRevenueMoney']) * HKD_Currency
                    
            elif GetMarketCodeUS(stock['StockCode']) == 'NAS' or GetMarketCodeUS(stock['StockCode']) == 'NYS':
                
                #if YF.get_asset_info(ChgTickerName(stock['StockCode'], 'US')) == 'Stock':
                StockNowMoney = StockNowMoney + float(stock['StockNowMoney']) * USD_Currency
                StockOriMoney = StockOriMoney + float(stock['StockOriMoney']) * USD_Currency
                StockRevMoney = StockRevMoney + float(stock['StockRevenueMoney']) * USD_Currency
                
        remained_Money = GetTotalBalance()    
        #print(StockNowMoney, StockOriMoney, StockRevMoney)
        #print(remained_Money)
        Balance = dict()
        Balance['StockMoney'] = StockNowMoney
        Balance['StockRevenue'] = StockRevMoney
        Balance['TotalMoney'] = remained_Money + StockNowMoney
        Balance['RemainMoney'] = remained_Money
