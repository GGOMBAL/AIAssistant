...
> **최신 업데이트 (2025-10-02)**: 3-Year Lookback 백테스팅 시스템 및 통합 MongoDB 데이터 로딩 구현 완료
...
## 🔄 3-Year Lookback 백테스팅 시스템 (2025-10-02 구현)

### 📊 **시스템 개요**

3-Year Lookback 시스템은 백테스트 실행 시 정확한 기술적 지표 계산을 위해 백테스트 시작일로부터 3년 전의 데이터까지 로드하여 지표를 계산한 후, 실제 백테스트 기간의 데이터만 추출하여 사용하는 시스템입니다.

### 🎯 **핵심 목적**

#### 문제점
- SMA200, Highest_2Y 등 장기 지표는 충분한 과거 데이터가 필요
- 백테스트 시작일부터 데이터를 로드하면 초기 지표값이 부정확
- refer 시스템과의 지표값 일치성 보장 필요
...
end_date = '2023-12-31'
data = load_data(start_date, end_date)  # 지표 계산에 불충분한 데이터

# 3-Year Lookback 방식 (정확)
lookback_date = start_date - timedelta(days=3*365)  # 2020-01-01
full_data = load_data(lookback_date, end_date)      # 충분한 데이터로 지표 계산
indicators = calculate_indicators(full_data)        # 정확한 지표 계산
backtest_data = indicators[start_date:end_date]     # 백테스트 기간만 추출
```
...
def load_daily_data_from_mongodb(symbol, start_date=None, end_date=None):
    """
    MongoDB에서 일간 데이터 로드 (날짜 필터링 지원)

    Args:
        symbol: 심볼명
        start_date: 시작일 (datetime 객체, None이면 전체)
        end_date: 종료일 (datetime 객체, None이면 전체)
    """
    query = {}
...
# 전체 데이터 로드
full_df = load_data_with_3year_lookback(symbol, lookback_date, end_date_dt)

# 지표 계산
df_with_indicators = calculate_all_indicators(full_df)

# 백테스트 기간 추출
backtest_df = df_with_indicators[
    (df_with_indicators.index >= start_date_dt) &
    (df_with_indicators.index <= end_date_dt)
].copy()
```

### 📈 **지표 계산 정확성 검증**

#### refer 시스템과의 일치성 검증 결과

| 지표명 | 공식 | refer 일치도 | 비고 |
|-------|------|-------------|------|
| **SMA200** | close.rolling(200).mean() | ✅ 100% | 200일 이동평균 |
| **SMA200_M** | SMA200 - SMA200.shift(1) | ✅ 100% | SMA200 모멘텀 |
| **Highest_2Y** | high.rolling(504).max() | ✅ 100% | 2년 최고가 (504일) |
| **52_H** | close.rolling(52).max() | ✅ 100% | 52주 최고가 |
| **1Year_H** | close.rolling(48).max() | ✅ 100% | 1년 최고가 (48주) |
| **2Year_H** | close.rolling(96).max() | ✅ 100% | 2년 최고가 (96주) |

#### 검증 방법
```python
# 수동 계산
sample_date = '2023-05-26'
sample_idx = full_df.index.get_loc(sample_date)
start_idx = max(0, sample_idx - 503)  # 504일 (2년)
period_data = full_df.iloc[start_idx:sample_idx+1]['high']
manual_highest_2y = period_data.max()

# 시스템 계산
calculated_highest_2y = df.loc[sample_date, 'Highest_2Y']

# 결과: 100% 일치
assert abs(manual_highest_2y - calculated_highest_2y) < 0.01
```

### ⚡ **성능 최적화**

#### 메모리 관리
- **점진적 로딩**: 필요한 날짜 범위만 로드
- **데이터 압축**: 사용하지 않는 컬럼 제거
- **메모리 해제**: 처리 완료 후 즉시 메모리 해제

#### 실행 시간 최적화
```python
# 로딩 시간 로그 예시
INFO: [3Y_LOOKBACK] Loading data from 2020-01-02 to 2023-12-31
INFO: [MONGODB_D] CRDO: Loaded 484 daily records (0.8s)
INFO: [INDICATORS] Calculated all indicators (1.2s)
INFO: [EXTRACT] Filtered to backtest period: 250 rows (0.1s)
# 총 소요시간: 2.1초
```

### 🔍 **신호 생성 검증**

#### CRDO 2023년 D신호 분석 결과
```
Main Conditions:
  SMA200_M > 0: 156/250 (62.4%)
  SMA200 < SMA50: 182/250 (72.8%)
  Combined: 136/250 (54.4%)

2Y Breakout Conditions:
  Highest_2Y <= Dhigh: 9/250 (3.6%)
  Highest_2Y == Highest_2Y.shift(5): 221/250 (88.4%)
  Final D signals: 0/250 (0.0%)
```

**결론**: refer와 Project 모두 동일하게 0개 D신호 생성 (정상)
...
이 3-Year Lookback 시스템을 통해 **정확하고 신뢰할 수 있는 백테스팅 환경**을 구축했습니다.