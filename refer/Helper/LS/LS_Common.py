...
def GetAppSecret(dist = "LS_KJM"):
    
    if dist == "LS_KJM":
        key = "LS_REAL_APP_SECRET"

    return stock_info[key]

def GetAccountNo(dist = "LS_KJM"):
...
def GetPrdtNo(dist = "LS_KJM"):
    
    if dist == "LS_KJM":
        key = "LS_REAL_ACNT_PRDT_CD"

    return stock_info[key]

def GetLsID(dist = "LS_KJM"):
...
def GetTokenPath(dist = "LS_KJM"):

    if dist == "LS_KJM":
        key = "LS_REAL_TOKEN_PATH"
       
    return stock_info[key]        

def MakeToken(dist = "LS_KJM"):

    headers = {"content-type":"application/x-www-form-urlencoded"}
    body = {
        "appkey":GetAppKey(dist), 
        "appsecretkey":GetAppSecret(dist),
...
def GetKRXOhlcv(stock_code,start_date,end_date,adVar=1):
    ...
    try:
        ...
        df = pd.concat([df_market_cap['capital'], df_ohlcv, df_p2[['per','pbr','eps','div']],df_type], axis=1)
    except:
        df = pd.concat([df_market_cap['capital'], df_ohlcv, df_p2[['per','pbr','eps','div']]], axis=1)
    df.index.name = "Date"
...
def GetSMA_MOM12(ohlcv,period,period2,st=100):
    ...
    if period == 200:
        ...
        SMA['SMA50_M'] = SMA['SMA50'] / SMA['SMA50'].shift(20*period2) - 1
        SMA['SMA50_RM'] = SMA['SMA50_M'] / SMA['SMA50_M'].mean()

        if st == 100: return SMA['SMA50_M']
        else:         return SMA['SMA50_M'].iloc[st]
