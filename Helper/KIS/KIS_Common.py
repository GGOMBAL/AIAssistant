...
# Removed trading API imports - backtesting only
...
# REMOVED: Trading-specific balance function - not needed for backtesting
# def GetBalanceKrwTotal():
#     # This function requires live trading API access
#     pass
...
def GetOhlcv(area, stock_code, limit = 500):
    ...
    try:
        # For backtesting, use alternative data sources only (no live trading API)
        if area == "US":
            # Use Yahoo Finance for US stocks in backtesting
            except_riase = False
            try:
            ...
            # Use KRX or FinanceDataReader for Korean stocks in backtesting
            except_riase = False
            try:
                df = GetOhlcv1(area,stock_code,Adjlimit)
            except Exception as e:
