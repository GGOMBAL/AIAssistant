...
def AutoLimit_System():
    ...
    bot_path_file_path = MainPath + "BotOrderListPath.json"
                        ...
                        if IsKRMarketOpen == True:
                            ...
                            if GapAmt != 0:
                                ...
                                if AutoLimitData['IsCancel'] == 'Y' or AutoLimitData['StockType'] == "DAY_END" or AutoLimitData['StockType'] == "DAY_END_TRY_ETF":
                                    ...
                                    print(">>>>>>>>>>>>>>>>>>>>> GOGO ORDER >>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
                                    telegram_bot.SendMessageWithRetry(f"재주문!! {str_msg}")
                                    ...
                                    try:
                                        ...
                                        print("Fail..", e)
                                        telegram_bot.SendMessageWithRetry(f"주문 실패: {str(e)}")
                                        telegram_bot.SendMessageWithRetry(f"주문 실패: {str(e)}")
                                        #주문이 실패했다
